...
/*
	...
	if (m_hFile)
	{
		...
		MsgShow(L"CSerial::Open - Port already opened",lpszDevice);
		return m_lLastError;
	}
	...
	if (m_hFile == INVALID_HANDLE_VALUE)
	{
		...
		MsgShow(L"CSerial::Open - Unable to open port",lpszDevice);
		return m_lLastError;
	}
...
*/
	...
	EscapeCommFunction(this->m_hFile, CLRDTR);
	EscapeCommFunction(this->m_hFile, CLRRTS);
...
bool CSerial::GetRLSD (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		AfxMessageBox(L"CSerial::GetRLSD - Unable to obtain the modem status!");
		return false;
	}
...
void CSerial::MsgShow(CString msg, CString arg)
{
	if (m_MsgShow==FALSE) return;
	CString mss;
	mss.Format(L"%s [%s]",msg,arg);
	AfxMessageBox(mss);
}


void CSerial::FlipRTS(void)
{
	if		(lRTS)		EscapeCommFunction(this->m_hFile, SETRTS);
	else				EscapeCommFunction(this->m_hFile, CLRRTS);
	lRTS^=TRUE;
}


void CSerial::FlipDTR(void)
{
	if		(lDTR)		EscapeCommFunction(this->m_hFile, SETDTR);
	else				EscapeCommFunction(this->m_hFile, CLRDTR);
	lDTR^=TRUE;
}


void CSerial::SetDTR(int par)
{
	if		(par)		EscapeCommFunction(this->m_hFile, SETDTR);
	else				EscapeCommFunction(this->m_hFile, CLRDTR);
}


BOOL CSerial::IsBusy(void)
{
	return (!m_hevtOverlapped || HasOverlappedIoCompleted((LPOVERLAPPED)0));
}


void CSerial::Scan(BYTE* list)
{
	HKEY hKey;
    DWORD i=0;
    TCHAR Name[4096];
...
void CSerial::SetRTS(int par)
{
	if		(par)		EscapeCommFunction(this->m_hFile, SETRTS);
	else				EscapeCommFunction(this->m_hFile, CLRRTS);
}


void CSerial::WriteHEX(char* str, int len)
{
	BYTE smb=0;
	BYTE ssmb[64];
	char chr[3];
	...
	for (i=0; i<len; i+=2)
	{
		memset(chr,0,3);
		memcpy(chr,&str[i],2);
		smb=strtol(chr,NULL,16);
		//this->Write(&smb,1);
		ssmb[j]=smb;
		j++;
	}

	this->Write(ssmb,len/2);
}

int psen=0;

void CSerial::PSEN(void)
{
	psen^=1;
	this->SetRTS(psen);
}
...
LONG CSerial::SetReadTimeOut(DWORD tout)
{
	COMMTIMEOUTS cto;
	m_lLastError = ERROR_SUCCESS;
	...
	cto.ReadIntervalTimeout=0;
    cto.ReadTotalTimeoutMultiplier=0;
    cto.ReadTotalTimeoutConstant=tout;
